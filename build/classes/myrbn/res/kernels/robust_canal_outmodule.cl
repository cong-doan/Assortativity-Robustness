...
__kernel void calOutModuleRobustness(__global const int *logicTables, __global const int *inv_logicTables,__global const int *nodesclusteridArr, 
			...
			int numStates,short typePert, short logicSize,int NumCluster)
{
	...
	for(iState = 0;iState<numStates;iState++)
	{
        ...
        double resultoutmodulerobustness=HammingDistance(transitionNetworkState,state,newstate,att1,att2,nodesclusteridArr,numPart,leftSize,numNodes,size1,size2,pos);
        sum=sum+resultoutmodulerobustness;
...
inline void converAttractorToStringArr(__global int *state,__global int *Attstring,__global const int *AttArr,short numPart, short leftSize,short numNodes,short AttSize)
{
        ...
        {
         i=j*numPart;
         converLongToStringArr(state,AttArr,i,numPart, leftSize,numNodes);
         //copy state to attstring
         index=j*numNodes;
            for(k=index;k<index+numNodes;k++)
             Attstring[k]=state[k-index];
        }   
