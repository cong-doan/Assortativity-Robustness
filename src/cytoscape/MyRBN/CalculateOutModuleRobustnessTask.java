 ...
 * @author Le Duc Hau
 */
public class CalculateOutModuleRobustnessTask  implements Task{
    ...
    /**/
    public CalculateOutModuleRobustnessTask() {
        this.error = false;
        this.errMsgs = "";
    ...
    public CalculateOutModuleRobustnessTask(int PerturbationType, boolean radCurrentState, boolean radAllPossibleState, String txtNumOfRandomStates) {
...
//                            }
                ...
                double [] noRobustStates_new = null;
                ...
                if(MyOpenCL.OPENCL_PLATFORM == MyOpenCL.GPU_PLATFORM)
                {
                    ...
                    noRobustStates_new = MyRBN.myopencl.calOutModuleRobustness_NestedCanalyzing(MyRBN.nodes, posSelectedNodes, allStates, PerturbationType, NumPossibleFunc, 1000,MyRBN.NumberOfCluster);
                     System.out.println("outmodulerobustness");
                    for(int ii=0;ii<noRobustStates_new.length;ii++)
                        System.out.print(noRobustStates_new[ii]+";");
                    
                    int [] nodeclusteridArr=MyRBN.myopencl.convertNodeClusterIDToIntArr(MyRBN.nodes);
                double robustoutmodule=0,r_inmodule=0;
                int size=0;
                for(i=0;i<MyRBN.NumberOfCluster;i++)
                {
                    r_inmodule=0;
                    size=0;
                    for(int j=0;j<MyRBN.nodes.size();j++)
                    {
                        if(nodeclusteridArr[j]==MyRBN.ac[i])
                        {
                            r_inmodule=r_inmodule+noRobustStates_new[j];
                            size++;
                        }
                    }
                    r_inmodule=r_inmodule/(double)size;                                          
                      robustoutmodule=robustoutmodule+r_inmodule;
                 }
                robustoutmodule=robustoutmodule/(double)MyRBN.NumberOfCluster;
                MyRBN.outmodulerobustness=robustoutmodule;
                System.out.println("robustoutmodule"+robustoutmodule);
                }
